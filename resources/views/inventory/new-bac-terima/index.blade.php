...
@section('title', 'New BAC Terima')

@section('content')
    <div id="content" class="app-content">

        {{ Breadcrumbs::render('new_bac_terima_index') }}

        @can('create bac terima')
            <div class="d-flex justify-content-end">
                <a href="{{ route('new-bac-terima.create') }}" class="btn btn-primary mb-3">
...
@push('js')
    ...
    <script>
        const action = '{{ auth()->user()->can('edit bac terima') || auth()->user()->can('delete bac terima') ? 'yes yes yes' : '' }}'
        ...
        $('#data-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('new-bac-terima.index') }}",
            columns: columns
        });
    </script>
@endpush
