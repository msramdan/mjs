...
@section('title', 'Edit ' . trans('sidebar.sub_menu.jurnal_umum'))

@section('content')
    <div id="content" class="app-content">
        {{ Breadcrumbs::render('jurnal_umum_edit') }}
            ...
            <form id="form-jurnal">
                <div class="panel-body">
                        ...
                        <div class="col-md-4 mb-1">
                            ...
                            <div class="input-group mb-3">
                                <input class="form-control" type="date" id="tanggal" name="tanggal"
                                    placeholder="Tanggal Transaksi" value="{{ $jurnalUmum->tanggal->format('Y-m-d') }}"
                                    required>
                            </div>
                        </div>
                        <div class="col-md-4 mb-1">
                            ...
                            <div class="input-group mb-3">
                                <input class="form-control" type="text" id="no_bukti" name="no_bukti"
                                    placeholder="No. Bukti" value="{{ $jurnalUmum->no_bukti }}" required>
                            </div>
                        </div>
                    </div>
                    ...
                    <table class="table table-bordered table-responsive" id="dynamic_field">
                        ...
                        <tbody>
                            @php
                                ...
                                $totalDebit = 0;
                            @endphp
                            @foreach ($relatedJurnals as $i => $jurnal)
                                <tr id="row{{ $i }}">
                                    <td>
                                    ...
                                    <td>
                                        <div class="form-group">
                                            <input type="text" name="deskripsi[]" class="form-control"
                                                placeholder="Deskripsi" value="{{ $jurnal->deskripsi }}" required />
                                        </div>
                                    </td>
                                    <td>
                                        <div class="form-group">
                                            <input type="text" class="form-control nominal_debit_text"
                                                id="debit{{ $i }}"
                                                value="{{ number_format($jurnal->debit) }}" />
                                            <input type="hidden" class="form-control nominal_debit"
                                                id="debit_asli{{ $i }}" name="debit[]"
                                                value="{{ $jurnal->debit }}" />
                                        </div>
                                    </td>
                                    <td>
                                        <div class="form-group">
                                            <input type="text" class="form-control nominal_kredit_text"
                                                id="kredit{{ $i }}"
                                                value="{{ number_format($jurnal->kredit) }}" />
                                            <input type="hidden" class="form-control nominal_kredit"
                                                id="kredit_asli{{ $i }}" name="kredit[]"
                                                value="{{ $jurnal->kredit }}" />
                                        </div>
                                    </td>
                                    <td>
                    ...
                    <table class="table" id="">
                        <tr>
                            <td style="width:20%">
                                <button id="simpan_data" type="submit" class="btn btn-danger me-2">
                            ...
                            <td style="width:20%;font-size:14px"> <b>Total Debit :</b> <br>
                                <span id="sum_debit">{{ number_format($totalDebit) }}</span>
                            </td>
                            <td style="width:20%;font-size:14px"> <b>Total Kredit :</b> <br>
                                <span id="sum_kredit">{{ number_format($totalKredit) }}</span>
                            </td>
                            <th style="width:5%"></th>
                        </tr>
                    </table>
        ...
        $("#simpan_data").click(function(e) {
            ...
            if (!tanggal) {
                ...
                $.ajax({
                    ...
                    url: "{{ route('jurnal-umum.update', $jurnalUmum->id) }}",
                    data: jurnal_umum,
                    dataType: "json",
